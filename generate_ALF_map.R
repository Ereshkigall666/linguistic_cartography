...
create_map <- function(source_file_path, dest_file_name, title = "", author = "", alf_ref = "", subtitle = "", web = T) {
  # load data
  ...
  g <- ggplot() +
    ...
    scale_shape_manual(values = c(17, 15, 43), "", limits = c("V", "C", "Other")) +
    scale_colour_manual(values = c("darkgreen", "red", "#b46cee"), limits = c("V", "C", "Other")) +
    theme(
  ...
  g <- g + annotate("text", x = -4, y = 51, label = title, size = 6, color = "blue", hjust = 0) +
    annotate("text", x = -4, y = 50.75, label = subtitle, size = 5, color = "black", hjust = 0) +
    annotate("text", x = -4, y = 50.515, label = paste("ALF c.", alf_ref, title, sep = " "), size = 4, color = "black", hjust = 0) +
    annotate("text", x = -4, y = 50.35, label = paste("© ", author), size = 3, color = "black", hjust = 0) +
...
########### tests ###############
create_map(source_file_path = "data/data_Vingt_standard_corr.csv", dest_file_name = "maps/vingt_map.png", title = "VINGT", author = "Marie-Sophie Desmarest", alf_ref = "1398", web = T, subtitle = "final-consonnant-pronunciation")

create_map(source_file_path = "data/data_Moins_standard_corr.csv", dest_file_name = "maps/moins_map.png", title = "MOINS", author = "Marie-Sophie Desmarest", alf_ref = "867", web = T, subtitle = "final-consonnant-pronunciation")
